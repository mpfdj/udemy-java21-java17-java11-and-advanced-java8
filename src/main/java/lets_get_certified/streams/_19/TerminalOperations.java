package lets_get_certified.streams._19;
...
//        doReduce3();
    ...
    public static void doCollect1(){
        ...
        StringBuilder word = Stream.of("ad", "jud", "i", "cate", "aaa", "bbb", "ccc")
        ...
        Stream<String> alphabet = Stream.of("aaa", "bbb", "ccc", "ddd");

       StringBuilder alphabetString = alphabet.collect(
                () -> new StringBuilder(),
                (sb, s) -> sb.append(s),
    ...
    public static void doReduce3(){
        ...
        // Use a Supplier to create a Stream, see https://www.baeldung.com/java-stream-operated-upon-or-closed-exception
        Supplier<Stream<String>> familyStream = () -> Stream.of("Miel", "Efril", "Kim", "Isabella");

        Optional<String> family = familyStream.get().reduce((s1, s2) -> s1.concat(s2));
        family.ifPresentOrElse(
                s -> System.out.println("Family: " + s),
                () -> System.out.println("No family")
        );


        Integer familyLength = familyStream.get().reduce(0,
                (n, s) -> {
                    System.out.printf("n: %s s: %s length: %s\n", n, s, s.length());
                    return n + s.length();
                },
                (n1, n2) -> {
    ...
    public static void doReduce2(){
        ...
        Optional<Integer> val2 = Stream.of(1,1,1)
                     .filter(n -> n > 1)      // val is 1 this way
                .reduce((a, b) -> a );// val is 1 this way too
        val2.ifPresentOrElse(
                i -> System.out.println("Accumulator returns " + i),
                () -> System.out.println("No result found due filter"));
...
//                       .reduce("nothing", (s, c) -> s + c);
        ...
        Integer sum = Stream.of(2,3,4)
                .reduce(0, (i1, i2)-> i1 + i2);
        System.out.println("Sum is " + sum);



        Integer sumWithFilter = Stream.of(2,3,4)
                .filter(i -> i > 5)
                .reduce(0, (i1, i2)-> i1 + i2);
        System.out.println("sumWithFilter is " + sumWithFilter + ", which is equal to the identity argument");
 ...
 //       for(Integer i : s){}// error: required array or Iterable
    ...
    public static void doMinAndMax(){ 
        ...
        Optional<Integer> max2 = Stream.of(4,6,2,12,9)
                ...
                .max((i1, i2) -> i1-i2);
        max2.ifPresent(i -> System.out.println("max is " + i));  // You should not see this
    }
}
