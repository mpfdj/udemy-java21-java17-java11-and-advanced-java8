...
//                                                         n -> n+2);   // next number 
    ...
    public static void fromMap() {
        ...
        namesToAges.put("Mike", 100);
        namesToAges.put("Mary", 24);
        namesToAges.put("Alice", 31);

        System.out.println(namesToAges);  // A map doesn't contain any duplicates!! Set as well.

        Stream<Map.Entry<String, Integer>> stream = namesToAges.entrySet().stream();
        stream.forEach(m -> System.out.printf("K: %s V: %s\n", m.getKey(), m.getValue()));
    }
